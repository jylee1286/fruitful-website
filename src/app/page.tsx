...
export default function Home() {
  return (
    <>
          ...
          <div className="grid lg:grid-cols-[1fr,auto] gap-16 lg:gap-20 items-center">
            <div className="max-w-2xl">
              ...
              <div className="reveal delay-3 flex flex-wrap items-center gap-4 mt-10">
                ...
                <span className="text-xs text-[#555] font-[family-name:var(--font-heading)]">7-day free trial &middot; No credit card</span>
              </div>
            </div>
        ...
        <div className="animate-marquee flex items-center gap-12 whitespace-nowrap">
          {[...Array(2)].map((_, i) => (
            <div key={i} className="flex items-center gap-12">
              <span className="text-xs text-[#444] font-[family-name:var(--font-heading)] tracking-widest uppercase">System-Level Blocking</span>
              <span className="text-[#FF6B9D] text-xs">●</span>
              <span className="text-xs text-[#444] font-[family-name:var(--font-heading)] tracking-widest uppercase">48 Collectible Characters</span>
              <span className="text-[#4ADE80] text-xs">●</span>
              <span className="text-xs text-[#444] font-[family-name:var(--font-heading)] tracking-widest uppercase">Trade With Friends</span>
              <span className="text-[#FF6B9D] text-xs">●</span>
              <span className="text-xs text-[#444] font-[family-name:var(--font-heading)] tracking-widest uppercase">Real-Time Tracking</span>
              <span className="text-[#4ADE80] text-xs">●</span>
              <span className="text-xs text-[#444] font-[family-name:var(--font-heading)] tracking-widest uppercase">Family Network</span>
              <span className="text-[#FF6B9D] text-xs">●</span>
              <span className="text-xs text-[#444] font-[family-name:var(--font-heading)] tracking-widest uppercase">Coin Rewards</span>
              <span className="text-[#4ADE80] text-xs">●</span>
            </div>
          ))}
        </div>
      ...
      {/* ═══════════════════ HOW IT WORKS — no cards, just clean typography ═══════════════════ */}
      <section id="how-it-works" className="relative py-28 sm:py-36">
        <div className="max-w-6xl mx-auto px-5 sm:px-8">
          ...
          <h2 className="reveal delay-1 text-3xl sm:text-4xl lg:text-[2.75rem] font-bold font-[family-name:var(--font-heading)] tracking-tight leading-tight mb-20">
            Three steps to <span className="gradient-text-pink">healthier habits.</span>
          </h2>

          {/* Steps — no cards, just numbered sections with dividing lines */}
          <div className="grid md:grid-cols-3 gap-x-12 gap-y-12">
            {[
              { num: "01", title: "Set the rules", desc: "Parents set daily screen time limits per app. Customize by day, block apps at bedtime, group apps together. Takes two minutes.", color: "text-[#FF6B9D]", line: "bg-[#FF6B9D]" },
              { num: "02", title: "Kids earn coins", desc: "Every day your child stays under their limits, they earn coins. More discipline means more coins to spend in the crate shop.", color: "text-[#4ADE80]", line: "bg-[#4ADE80]" },
              { num: "03", title: "Collect & trade", desc: "Coins open fruit crates that reveal character cards. 48 to collect across 12 families. Four rarity tiers each. Trade with friends.", color: "text-[#FF8FAE]", line: "bg-gradient-to-r from-[#FF6B9D] to-[#4ADE80]" },
            ].map((step, i) => (
              <div key={step.num} className={`reveal delay-${i + 1}`}>
                ...
                <h3 className="text-xl font-bold font-[family-name:var(--font-heading)] mt-2 mb-3">{step.title}</h3>
      ...
      {/* ═══════════════════ FOR PARENTS — asymmetric layout ═══════════════════ */}
      <section className="relative py-28 sm:py-36 overflow-hidden">
        <div className="watermelon-divider" />
        <div className="pt-28 sm:pt-36" />

        <div className="max-w-6xl mx-auto px-5 sm:px-8">
          <div className="grid lg:grid-cols-[1fr,1.1fr] gap-16 lg:gap-24 items-start">
            {/* Phone mockup — no card wrapper */}
            <div className="reveal-left relative flex justify-center lg:sticky lg:top-28">
              <div className="relative w-[260px] sm:w-[280px]">
                <div className="rounded-[2.5rem] border-[6px] border-[#1A1A1A] bg-[#0A0A0A] overflow-hidden shadow-2xl shadow-black/60">
                  ...
                  <div className="pt-9 pb-5 px-4 min-h-[480px]">
                    <p className="text-[10px] text-[#555] uppercase tracking-wider font-[family-name:var(--font-heading)]">Today&apos;s Screen Time</p>
                    <div className="flex items-end justify-between mt-1.5 mb-4">
                      <p className="text-2xl font-bold font-[family-name:var(--font-heading)]">2h 14m</p>
                    ...
                    {[
                      ...
                      <div key={app.name} className="flex items-center gap-2 py-2.5 border-b border-white/[0.03] last:border-0">
                        <div className={`w-6 h-6 rounded-md ${app.color} opacity-80 shrink-0`} />
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between mb-1">
                            <span className="text-[10px] font-medium">{app.name}</span>
                            <span className="text-[9px] text-[#555]">{app.time}/{app.limit}</span>
                          </div>
                    ...
                    <div className="mt-3 py-2 text-center border border-dashed border-[#222] rounded-lg">
                      <span className="text-[10px] text-[#444]">+ Add App</span>
                    </div>
                  </div>
                </div>
                <div className="absolute bottom-2 left-1/2 -translate-x-1/2 w-24 h-1 rounded-full bg-[#222]" />

                {/* Floating mascot peeking from behind phone */}
                <div className="absolute -top-8 -right-12 animate-float-offset">
                  <Image src="/images/mascot-explore.png" alt="" width={80} height={80} className="drop-shadow-[0_0_20px_rgba(255,107,157,0.2)]" />
                </div>
              </div>
            </div>

            {/* Copy — clean, no card wrappers */}
            <div>
              ...
              <p className="reveal delay-2 text-[#777] text-base leading-relaxed mb-10 max-w-md">
                ...
                {[
                  ...
                  <div key={item.title} className={`reveal delay-${Math.min(i + 2, 6)}`}>
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <p className="text-sm font-semibold font-[family-name:var(--font-heading)] mb-0.5">{item.title}</p>
      ...
      {/* ═══════════════════ FOR KIDS — dark green tinted section ═══════════════════ */}
      <section className="relative py-28 sm:py-36 overflow-hidden bg-[#040804]">
        <div className="watermelon-divider" />
        <div className="pt-28 sm:pt-36" />

        {/* Subtle green tint ambient */}
        ...
        <div className="relative max-w-6xl mx-auto px-5 sm:px-8">
          <div className="text-center mb-20">
            ...
            <p className="reveal delay-2 text-[#777] text-base max-w-xl mx-auto leading-relaxed">
          ...
          {/* Crate showcase — interactive scale-on-scroll */}
          <CrateReveal />

          {/* Character families — minimal, no card wrappers */}
          <div className="reveal mt-24">
            <p className="text-center text-[10px] font-bold text-[#444] uppercase tracking-[0.25em] mb-8 font-[family-name:var(--font-heading)]">
              12 Families &middot; 4 Rarities &middot; 48 Cards
            </p>
            <div className="flex flex-wrap justify-center gap-x-6 gap-y-4 max-w-3xl mx-auto">
              {["Annie","Bobby","Charlie","Gary","Sally","Lenny","Wade","Penny","Bella","Marty","Kenny","Daisy"].map((name, i) => {
                const dotColors = ["bg-pink-400","bg-blue-400","bg-amber-400","bg-green-400","bg-purple-400","bg-red-400","bg-cyan-400","bg-fuchsia-400","bg-rose-400","bg-lime-400","bg-sky-400","bg-yellow-400"];
                return (
                  <div key={name} className={`reveal delay-${(i % 6) + 1} flex items-center gap-2 group cursor-default`}>
                    <span className={`w-2.5 h-2.5 rounded-full ${dotColors[i]} opacity-70 group-hover:opacity-100 group-hover:scale-125 transition-all duration-300`} />
                    <span className="text-xs text-[#555] font-[family-name:var(--font-heading)] font-medium group-hover:text-[#999] transition-colors duration-300">{name}</span>
                  </div>
                );
              })}
            </div>
            <div className="flex items-center justify-center gap-3 mt-6">
              {[
                { tier: "Common", color: "text-[#666]" },
                { tier: "Rare", color: "text-blue-400" },
                { tier: "Epic", color: "text-purple-400" },
                { tier: "Legendary", color: "text-amber-400" },
              ].map((t) => (
                <span key={t.tier} className={`text-[10px] font-bold uppercase tracking-wider ${t.color} font-[family-name:var(--font-heading)]`}>
                  {t.tier}
                </span>
              ))}
            </div>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-x-10 gap-y-12">
            {[
              ...
              <div key={f.title} className={`reveal delay-${(i % 4) + 1}`}>
                ...
                <h3 className="text-sm font-bold font-[family-name:var(--font-heading)] mt-3 mb-1.5">{f.title}</h3>
      ...
      {/* ═══════════════════ PRICING — warm glow panel, no card ═══════════════════ */}
        ...
        <div className="relative max-w-lg mx-auto px-5 sm:px-8 text-center">
          ...
          <div className="reveal-scale">
            ...
            <div className="flex items-end justify-center gap-1 mb-1">
              ...
              <span className="text-[#555] mb-3 text-sm">/month</span>
            </div>
            <p className="text-xs text-[#555] mb-10 font-[family-name:var(--font-heading)]">after 7-day free trial</p>
      ...
      <section className="relative py-28 sm:py-36 overflow-hidden">
        ...
        <div className="relative max-w-2xl mx-auto px-5 sm:px-8 text-center">
          ...
          <p className="reveal delay-2 text-[#777] text-base mb-8">Free for 7 days. Cancel anytime. Your family deserves this.</p>
